...
int MtfList<Object>::find( const Object &obj ) {
  ...
  if ( found->next != NULL )
    found->next->prev = found->prev; // Re-link the next node of the found to the previous node of found

  // insert found between header and top
  found->next = top;                  
  top->prev = found;                // top != null
  found->prev = DList<Object>::header; // previous node of found is now the dummy
  DList<Object>::header->next = found; // dummy next now point to found 
